...
function parseScriptsFromHTML(html) {
  const scripts = [];
...
async function searchScripts(queries) {
  const allScripts = [];
  ...
  const queryResults = await Promise.all(
    queries.map(async (q) => {
      try {
        ...
        return { scripts: [], pagesFetched: 0, error: err.message };
      }
    })
  );
...
async function getScriptDetails(scriptUrls) {
  const results = await Promise.all(
